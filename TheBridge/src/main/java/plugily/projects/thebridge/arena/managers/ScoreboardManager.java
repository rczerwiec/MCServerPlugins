...
public class ScoreboardManager {
  ...
  private String formatBase(String line, User user, Base base) {
    String formattedLine = line;
    String color = "X";
    //Bukkit.broadcastMessage("Nie zwracaj uwagi na to - "+base.getColor());
    ...
    if(line.contains("%FORMATTED_POINTS%")) {
      ...
      formattedLine = StringUtils.replace(formattedLine, "%FORMATTED_POINTS%", "§6"+base.getPoints()+"/"+arena.getOption(ArenaOption.MODE_VALUE));
    }
    formattedLine = chatManager.colorRawMessage(formattedLine);
    if(!baseYou)
